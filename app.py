...
def webhook():
    ...
    try:
        ...
        if message_id:
            ...
            # Evita acúmulo infinito de IDs na memória
...
def ask_openai(number, text):
    ...
    try:
        r = requests.post(
            ...
            json=b,
            headers=h,
            timeout=30  # evita travar indefinidamente
        )
        r.raise_for_status()  # lança erro se status for 4xx ou 5xx

        resultado = r.json()

        # Verifica se choices existe e não está vazio
        ...
        print(f"ERRO HTTP OpenAI: {e} | Resposta: {r.text}")
        return "Estou com uma instabilidade agora. Tente novamente em instantes! 😊"

    except Exception as e:
        print(f"ERRO inesperado OpenAI: {e}")
...
def send(number, text):
    ...
    try:
        r = requests.post(f"{BASE}/send/text", json=b, headers=h, timeout=15)
